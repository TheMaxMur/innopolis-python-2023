...
def write_result_to_json(result: list, name: str) -> None:
    date = str(datetime.now()).split(" ")[0]

    with open(f"results/{name}.json", "w", encoding="utf-8") as f:
        json.dump(result, f, ensure_ascii=False, indent=2)

    print(f"Данные записаны в results/{name}.json")
...
async def main() -> int:
    ...
    write_result_to_json(result, "get_podcategorii")
    ...
    write_result_to_json(test, "get_product")
    ...
    write_result_to_json(result, "get_product_info")
